...
from src.teams.resources.drivers_license_eval import generate
...
#     driver_license_number: Annotated[str, "claimant's driver license number."]
    image_path: Annotated[str, "Path to the driver's license image."]
):
    """
    Check the driver's license status.
    """
    """
    Checks the status of the driver's license by analyzing the provided image.
    
    Args:
        image_path (str): The path to the driver's license image.

    Returns:
        dict: A dictionary containing the verification status and message.
    """
    try:
        # Generate the response using the image verification logic
        response = generate(image_path)

        # Check the response and return the appropriate message
        if response and response[0]['isValid'] == "valid":
            return {
                "status": "valid",
                "message": "The driver's license is valid."
            }
        else:
            return {
                "status": "invalid",
                "message": "The driver's license is invalid."
            }
    except Exception as e:
        return {
            "status": "error",
